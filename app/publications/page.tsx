...
const CategoryNav = ({ activeCategory, onSelectCategory }: any) => {
  const { t } = useTranslation();
  return (
    <div className="w-full overflow-x-auto pb-1 pt-2 scrollbar-hide">
        ...
        {categories.map((cat) => {
          ...
          return (
            ...
            >
              ...
              <span className="whitespace-nowrap">{t(`publications.categories.${cat.id}`)}</span>
            </button>
          );
        })}
      </div>
              ...
              <div className="col-span-2 space-y-4">
                 ...
                 <div className="p-4 bg-slate-50 rounded-lg border border-slate-100">
                    ...
                    <div className="flex items-center gap-2 text-slate-800 font-medium">
                       {category.icon}
                       <span>{t(`publications.categories.${category.id}`)}</span>
                    </div>
                 </div>
              </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 flex flex-col">
      ...
      <div className="flex-1 transition-all duration-300">
        ...
        {/* Top Header Bar */}
        <header className="sticky top-0 z-20 bg-white/80 backdrop-blur-md border-b border-slate-200 px-6 py-4 flex flex-col gap-4">
           <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
             <div>
               <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
                 {t(`publications.categories.${activeCategory.id}`)}
               </h1>
             </div>
